...
노트북에서는 품목별 가격 스케일 차이가 크기 때문에, 절대 오차보다 **상대 오차 중심의 비교**가 적절하다고 판단해 MdAPE 기반으로 모델 성능을 비교했습니다.
...
- 품목별 예측 결과 비교
- 모델별 성능 요약표 제공
- 비개발자도 쉽게 확인할 수 있는 웹 형태로 배포

### 배포 목적
- 분석 결과를 노트북에만 남기지 않고 서비스 형태로 전달
- 데이터 분석 결과를 의사결정자/현업이 직접 확인할 수 있도록 구현
- 포트폴리오 관점에서 **분석 -> 모델링 -> 배포**까지의 전체 역량을 보여주기 위함
